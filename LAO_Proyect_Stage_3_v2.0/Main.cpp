...
bool parseComparison(const char *ch) {
	...
	if (tolower(ch[logicalCompareLoc + 2]) == 'g' && tolower(ch[logicalCompareLoc + 3]) == 't') {
		if (isVariable(value1) && !isVariable(value2)) {// one is variable
			if (isNumber(value2)) {
		...
		else if (isVariable(value2) && !isVariable(value1)) {// one is variable
			if (isNumber(value1)) {
				if (isIntegerVariable(value1)) {
					int intValue2;
			...
			if (isIntegerVariable(value1)) {
				...
				int intValue2;
				findIntegerVariable(value2, intValue2);
				return Comparison<int>::isGt(intValue1, intValue2); // Need to change value 2 to use variable value
			}
